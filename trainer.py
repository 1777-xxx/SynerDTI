...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader,  discriminator=None,
        ...
        test_metric_header = ["# Best Epoch", "AUROC", "AUPRC", "F1", "Sensitivity", "Specificity", "Accuracy",
                              "Threshold", "Test_loss","Precision","recall"]
    ...
    def train(self):
        ...
        test_lst = ["epoch " + str(self.best_epoch)] + list(map(float2str, [auroc, auprc, f1, sensitivity, specificity,
                                                                            accuracy, thred_optim, test_loss,precision,recall]))
        self.test_table.add_row(test_lst)
        print('Test at Best Model of Epoch ' + str(self.best_epoch) + ' with test loss ' + str(test_loss), " AUROC "
              + str(auroc) + " AUPRC " + str(auprc) + " f1 " + str(f1)+" Sensitivity " + str(sensitivity) + " Specificity " +
              str(specificity) + " Accuracy " + str(accuracy) + " Precision " + str(precision)+" Thred_optim " + str(thred_optim)+ " recall " + str(recall))
        ...
        if self.experiment:
            ...
            self.experiment.log_metric("test_recall", self.test_metrics["recall"])
    ...
    def test(self, dataloader="test"):
        ...
        if dataloader == "test":
            ...
            return auroc, auprc, np.max(f1[5:]), sensitivity, specificity, accuracy, test_loss, thred_optim, precision1,recall1
        else:
